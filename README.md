...
## 코드
1. backteser 폴더내에는 초단타, 단기 전략용 백테스터가 포함되어 있습니다.
- backtester_1.py는 10시 이전 초단타 전략을 백테스트하고 변수값을 최적화하는 코드입니다.
- backtester_2.py는 10시 이후 초단타 전략을 백테스트하고 변수값을 최적화하는 코드입니다.
...
- backtester_1m.py 및 backtester_2m.py는 전략 변수값을 고정하여 백테스트할 수 있습니다.
...
2. collector 폴더내에는 틱데이터 수집코드가 포함되어 있습니다.
- window.py 를 실행하면 초당 틱데이터를 수집할 수 있습니다.
...
- strategy.py는 단타 전략 연산용 클래스 파일입니다.
...
6. updater 폴더는 단기 전략용 업데이터가 포함되어 있습니다.
7. 배치파일 동작 순서
- 두번째 계정으로 버전업그레이드 실행
- 두번째 계정으로 자동로그인 설정
- 틱수집 프로그램 실행
...
- 10시 이전 초단타 전략 변수 최적화용 백테스터 실행
- 10시 이후 초단타 전략 변수 최적화용 백테스터 실행
- 프로그램 종료
## 설정방법
...
- 한종목 집중모드에서는 차트 호가창 이외에 뉴스, 공시, 재무재표, 체결강도 추이까지 표시됩니다.
